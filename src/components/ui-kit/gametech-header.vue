<script>
import { defineComponent } from 'vue'

export default defineComponent({
  name: "gametech-header",

  data() {
    return {
      isActive: false,
    }
  },

  props: {
    favoritesLength: {
      type: Number,
      default() {
        return 0
      },
    },

    cartLength: {
      type: Number,
      default() {
        return 0
      },
    },
  },

  mounted() {
    document.addEventListener("scroll", this.updateHeaderStatus)
    document.addEventListener("resize", this.updateHeaderStatus)
  },

  beforeUnmount() {
    document.removeEventListener("scroll", this.updateHeaderStatus)
    document.removeEventListener("resize", this.updateHeaderStatus)
  },

  methods: {
    updateHeaderStatus() {
      this.isActive = window.pageYOffset > 0
    },
  },
})
</script>

<template>
  <header
      class="gametech-header"
      :class="{
        'active': isActive,
      }"
  >
    <nav class="left">
      <router-link to="/catalog">
        <h3>КАТАЛОГ</h3>
      </router-link>

      <router-link to="/contacts">
        <h3>КОНТАКТЫ</h3>
      </router-link>
    </nav>

    <router-link to="/">
      <h1 class="title">GAME<span class="red-text">TECH</span></h1>
    </router-link>

    <nav class="right">
      <router-link to="/favorites" class="favorites-wrapper">
        <svg class="favorites" width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M11.223 2.43177C11.5066 2.27421 11.6484 2.19543 11.7985 2.16454C11.9315 2.13721 12.0685 2.13721 12.2015 2.16454C12.3516 2.19543 12.4934 2.27421 12.777 2.43177L20.177 6.54288C20.4766 6.70928 20.6263 6.79248 20.7354 6.91082C20.8318 7.01551 20.9049 7.13959 20.9495 7.27477C21 7.42756 21 7.59889 21 7.94153V16.0586C21 16.4013 21 16.5726 20.9495 16.7254C20.9049 16.8606 20.8318 16.9847 20.7354 17.0893C20.6263 17.2077 20.4766 17.2909 20.177 17.4573L12.777 21.5684C12.4934 21.726 12.3516 21.8047 12.2015 21.8356C12.0685 21.863 11.9315 21.863 11.7985 21.8356C11.6484 21.8047 11.5066 21.726 11.223 21.5684L3.82297 17.4573C3.52345 17.2909 3.37369 17.2077 3.26463 17.0893C3.16816 16.9847 3.09515 16.8606 3.05048 16.7254C3 16.5726 3 16.4013 3 16.0586V7.94153C3 7.59889 3 7.42756 3.05048 7.27477C3.09515 7.13959 3.16816 7.01551 3.26463 6.91082C3.37369 6.79248 3.52345 6.70928 3.82297 6.54288L11.223 2.43177Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
          <path fill-rule="evenodd" clip-rule="evenodd" d="M11.9966 9.06799C10.9969 7.89928 9.32987 7.58491 8.07735 8.65509C6.82482 9.72528 6.64848 11.5146 7.6321 12.7803C8.26211 13.591 9.87558 15.0943 10.9542 16.0704C11.3127 16.3948 11.4919 16.557 11.7066 16.6221C11.8911 16.678 12.102 16.678 12.2866 16.6221C12.5012 16.557 12.6805 16.3948 13.0389 16.0704C14.1176 15.0943 15.731 13.591 16.3611 12.7803C17.3447 11.5146 17.1899 9.71402 15.9158 8.65509C14.6417 7.59616 12.9963 7.89928 11.9966 9.06799Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>

        <h2 class="counter" v-if="favoritesLength">{{ favoritesLength }}</h2>
      </router-link>

      <router-link to="/cart" class="cart-wrapper">
        <svg class="cart" width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M6.29977 5H21L19 12H7.37671M20 16H8L6 3H3M9 20C9 20.5523 8.55228 21 8 21C7.44772 21 7 20.5523 7 20C7 19.4477 7.44772 19 8 19C8.55228 19 9 19.4477 9 20ZM20 20C20 20.5523 19.5523 21 19 21C18.4477 21 18 20.5523 18 20C18 19.4477 18.4477 19 19 19C19.5523 19 20 19.4477 20 20Z" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        </svg>

        <h2 class="counter" v-if="cartLength">{{ cartLength }}</h2>
      </router-link>
    </nav>
  </header>
</template>

<style scoped lang="sass">
.gametech-header
  width: 100%
  max-width: 1280px
  display: flex
  align-items: center
  padding: 10px 30px
  background-color: #ffffff
  position: sticky
  top: 0
  z-index: 100
  border: 1px solid transparent
  border-top: none !important

  &.active
    border: 1px solid gray
    +border-radius(0 0 10px 10px)

  nav
    width: 100%
    display: flex
    gap: 20px
    font-size: 30px
    
    > *
      cursor: pointer

      &:hover
        color: $red

    &.left
      justify-content: flex-start

    &.right
      justify-content: flex-end

    .counter
      position: absolute
      top: -5px
      right: -5px
      font-size: 10px
      background-color: $red
      color: white
      width: 20px
      height: 20px
      +border-radius(50%)
      padding: 5px
      display: flex
      justify-content: center
      align-items: center

    .favorites-wrapper
      position: relative

      .favorites
        width: 40px
        height: 40px
        stroke: $red

        &:hover
          path
            stroke: $red

    .cart-wrapper
      position: relative

      .cart
        width: 40px
        height: 40px
        stroke: $red

        &:hover
          path
            stroke: $red
    
  .title
    font-size: 50px

    .red-text
      color: $red
</style>